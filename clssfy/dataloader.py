...
class PropDataset(data.Dataset):
    def __init__(self, fpath, newfpath, IsTest=False):
        ...
        if IsTest:
            ...
            new_words = json.load(open(newfpath))
            new_tags = [['O'] * len(x) for x in new_words]
            new_ids = ['deprop'] * len(new_words)

            flat_words.extend(new_words)
            flat_tags.extend(new_tags)
            flat_ids.extend(new_ids)
            
            zipped_data = list(zip(flat_words, flat_tags, flat_ids))
            random.shuffle(zipped_data)

            flat_words, flat_tags, flat_ids = zip(*zipped_data)
        if hp.dummy:
            flat_words, flat_tags, flat_ids = flat_words[:2], flat_tags[:2], flat_ids[:2]
        print(newfpath, len(flat_words), flat_words[:2], len(flat_ids), flat_ids[:2])
        print('\n')
...
def pad(batch):
    ...
    is_heads = f(2)
    seqlen = f(6)
    maxlen = max([len(sample[1]) for sample in batch]) + 5

    f = lambda x, seqlen: [sample[x][:maxlen-2] + [0] * (seqlen - len(sample[x][:maxlen-2])) for sample in batch] # 0: <pad>
